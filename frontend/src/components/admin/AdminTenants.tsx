...
import { plansService, type SubscriptionPlanRead } from "@/services/plans.service";
...
export function AdminTenants() {
  ...
  const saveTenantPlan = async () => {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : "Error al asignar plan");
    } finally {
      setSavingPlan(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-[40px] p-10 shadow-sm border border-slate-100 flex flex-col flex-1 overflow-hidden relative">
                ...
                <div className="flex items-center gap-4 opacity-80 group-hover:opacity-100 transition-opacity">
                   ...
                   <div className="flex items-center gap-1">
                      {t.name === "Nodo Principal" ? (
                         <div className="flex items-center justify-center p-1.5 pr-2 text-slate-300" title="Empresa protegida del sistema">
                            <ShieldAlert size={18} />
                         </div>
                      ) : (
                         <>
      ...
      {/* Modal Suscripción */}
      {planModalTenant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setPlanModalTenant(null)}>
                ...
                <label key={p.id} className={`flex items-start gap-4 cursor-pointer p-4 rounded-2xl border-2 transition-all ${selectedPlanId === p.id ? 'border-[#111111] bg-slate-50 shadow-sm' : 'border-transparent bg-slate-50/50 hover:bg-slate-50'}`}>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <p className="font-bold text-[#111111] text-lg">{p.name}</p>
