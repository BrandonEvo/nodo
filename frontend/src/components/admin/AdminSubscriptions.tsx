...
export function AdminSubscriptions() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPlanId) {
        await subscriptionsService.update(editingPlanId, {
          name: name.trim(),
          price: Number(price),
          currency: "GTQ",
      ...
      alert(e instanceof Error ? e.message : "Error al guardar el plan");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Desactivar este plan? Podrá seguir en la base de datos pero de baja lógica.")) return;
    try {
      await subscriptionsService.delete(id);
      await loadData();
    } catch (e: unknown) {
      alert(e instanceof Error ? e.message : "Error al desactivar el plan");
    }
  };
  ...
  const handleReactivate = async (id: string) => {
    try {
      ...
      alert(e instanceof Error ? e.message : "Error al reactivar el plan");
    }
  };
  ...
  const handleHardDelete = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(e.response?.data?.detail || e.message || "Error al destruir el plan");
    } finally {
      setDeleting(false);
    }
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8 shrink-0">
        <div>
          <h2 className="text-2xl font-black text-[#111111] tracking-tight flex items-center gap-3">
            <CreditCard className="text-[#69E7A8] w-8 h-8" /> Suscripciones y Planes
          </h2>
          <p className="text-slate-500 text-sm mt-1">Configura qué módulos están incluidos en cada plan de facturación.</p>
        </div>
        <Button
          onClick={() => openForm()}
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        {loading ? (
          ...
          <div className="text-center py-20 bg-slate-50 rounded-[30px] border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 font-medium">No hay planes creados.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {plans.map(plan => (
              <div key={plan.id} className={`border border-slate-200 rounded-[32px] p-8 relative flex flex-col ${!plan.is_active ? 'bg-slate-50' : 'hover:border-[#111111]/20'} transition-colors`}>
                <div className="absolute top-6 right-6 flex items-center gap-1">
                  <button
                    onClick={() => openForm(plan)}
                    className="p-2 text-slate-400 hover:text-[#111111] hover:bg-slate-100 rounded-full transition"
                    title="Editar plan"
                  >
                    <Pencil size={18} />
                  </button>
                  {plan.is_active ? (
                    <button
                      ...
                      className="p-2 text-orange-400 hover:text-white hover:bg-orange-500 rounded-full transition"
                    ...
                    <>
                      <button
                        ...
                        className="p-2 text-green-500 hover:text-white hover:bg-green-500 rounded-full transition bg-green-50"
                        title="Reactivar plan"
                      >
                        <PowerOff size={18} />
                      </button>
                      <button
                        onClick={() => setHardDeletingId(plan.id)}
                        className="p-2 text-red-500 hover:text-white hover:bg-red-600 rounded-full transition bg-red-50"
                ...
                <div className="flex justify-between items-start mb-6 pr-32">
                  <div>
                    <h3 className={`text-2xl font-black ${!plan.is_active ? 'text-slate-400' : 'text-[#111111]'}`}>{plan.name}</h3>
                    <div className="flex items-baseline gap-1 mt-2">
                      <span className={`text-3xl font-black ${!plan.is_active ? 'text-slate-300' : ''}`}>Q{plan.price}</span>
                ...
                <div className="mb-6">
                  {plan.is_active ? (
                    <span className="bg-[#69E7A8]/20 text-[#111111] px-3 py-1 rounded-full text-xs font-black tracking-widest uppercase">
                      Activo
                    </span>
                  ) : (
                    <span className="bg-slate-200 text-slate-500 px-3 py-1 rounded-full text-xs font-black tracking-widest uppercase">
                      Inactivo
                    </span>
                  )}
                </div>
                ...
                <div className={`flex-1 ${!plan.is_active ? 'opacity-50' : ''}`}>
                  <p className="text-xs font-bold text-slate-400 tracking-widest uppercase mb-4">Módulos Incluidos</p>
                  {!plan.module_ids || plan.module_ids.length === 0 ? (
                      ...
                      {plan.module_ids.map(mid => {
                        ...
                        return (
                          <li key={mid} className="flex items-center gap-3 text-sm font-semibold text-slate-700">
                            <Check size={16} className={plan.is_active ? "text-[#69E7A8]" : "text-slate-300"} />
                            {mod ? mod.name : mid}
                          </li>
                        )
                      })}
      ...
      {hardDeletingId && (
        <div className="absolute inset-0 bg-[#111111]/80 backdrop-blur-md z-30 flex items-center justify-center p-4">
          <form onSubmit={handleHardDelete} className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200 text-center">
            ...
            <p className="text-slate-500 text-sm mb-6">Esta acción borrará el plan de la base de datos de manera irreversible. Requiere autorización maestra.</p>
            ...
            <div className="flex gap-2">
              <Button type="button" variant="outline" onClick={() => setHardDeletingId(null)} className="flex-1 h-12 rounded-xl text-slate-500 font-bold border-slate-200">
                CANCELAR
              </Button>
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto flex flex-col gap-6 pr-2 custom-scrollbar">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="h-12 rounded-2xl bg-slate-50 border-slate-200 pl-8 pr-5 focus-visible:ring-[#111111]/5"
                    required
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-slate-50 p-4 rounded-[24px] border border-slate-100 space-y-2">
                  {modules.length === 0 ? (
                    <p className="text-xs text-slate-500 px-2 py-4 text-center">No hay módulos disponibles en el sistema.</p>
                  ) : (
                    modules.map(m => (
