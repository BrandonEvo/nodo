...
import { Plus, Package, CheckCircle2, Pencil, PowerOff, X, Trash2, ShieldAlert } from "lucide-react";
...
export function AdminModules() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : "Error al guardar módulo");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Desactivar este módulo? Quedará inactivo.")) return;
    try {
      await modulesService.update(id, { is_active: false });
      await loadData();
    } catch (e: unknown) {
      alert(e instanceof Error ? e.message : "Error al desactivar módulo");
    }
  };
  ...
  const handleReactivate = async (id: string) => {
    try {
      ...
      alert(e instanceof Error ? e.message : "Error al reactivar módulo");
    }
  };
  ...
  const handleHardDelete = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(e.response?.data?.detail || e.message || "Error al destruir el módulo");
    } finally {
      setDeleting(false);
    }
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8 shrink-0">
        <div>
          <h2 className="text-2xl font-black text-[#111111] tracking-tight flex items-center gap-3">
            <Package className="text-[#69E7A8] w-8 h-8" /> Módulos Globales
          </h2>
          <p className="text-slate-500 text-sm mt-1">Registra todas las funcionalidades o apps disponibles en tu ecosistema.</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <Plus size={18} className="mr-2" /> CREAR MÓDULO
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {modules.map((m) => (
              <div key={m.id} className={`p-6 bg-white border border-slate-100 rounded-[24px] shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group ${m.is_active === false ? 'bg-slate-50 opacity-60' : ''}`}>
                <div className="absolute top-0 right-0 p-4 opacity-5 bg-[#69E7A8] rounded-bl-[40px] w-24 h-24 transition-transform group-hover:scale-110"></div>
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-1 z-20">
                    ...
                    {m.is_active !== false ? (
                      ...
                      <>
                        ...
                        <button 
                          onClick={() => setHardDeletingModuleId(m.id)}
                ...
                <div className="flex items-start justify-between relative z-10 pr-12">
                   <div>
                      <h4 className={`font-black text-lg leading-tight ${m.is_active === false ? 'text-slate-500' : 'text-[#111111]'}`}>{m.name}</h4>
             ...
             <div className="w-full max-w-md h-full bg-white border-l border-slate-200 shadow-2xl flex flex-col p-8 animate-in slide-in-from-right duration-300">
                ...
                <form onSubmit={handleSave} className="flex-1 overflow-y-auto flex flex-col gap-6 pr-2 custom-scrollbar">
                   ...
                   <div className="mt-auto pt-8">
                      <Button 
                        type="submit" 
                        disabled={saving || !newModuleName.trim() || !newModuleCode.trim()}
       ...
       {hardDeletingModuleId && (
        <div className="absolute inset-0 bg-[#111111]/80 backdrop-blur-md z-30 flex items-center justify-center p-4">
           <form onSubmit={handleHardDelete} className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200 text-center">
             ...
             <p className="text-slate-500 text-sm mb-6">Esta acción borrará el módulo. Eliminará silenciosamente de la base de datos a sus planes, roles y membresías asociadas. ¿Confirma?</p>
             ...
             <div className="flex gap-2">
               <Button type="button" variant="outline" onClick={() => setHardDeletingModuleId(null)} className="flex-1 h-12 rounded-xl text-slate-500 font-bold border-slate-200">
                 CANCELAR
               </Button>
