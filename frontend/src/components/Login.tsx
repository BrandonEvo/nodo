...
import { CheckCircle2, EyeOff, Eye, Mail, Lock, Store, ArrowLeft, Loader2 } from "lucide-react"
...
export function Login({ onLoginSuccess }: LoginProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      // register-workspace ahora emite cookies de sesión automáticamente
      await authService.registerWorkspace({ tenant_name: tenantName, email, password })
      onLoginSuccess()
    } catch (err: any) {
        ...
        <div className="w-full max-w-[440px] transition-all duration-500">
          ...
          {view === 'forgot' && (
            <div className="w-full animate-in fade-in slide-in-from-left-4 duration-300">
              ...
              <p className="text-sm text-gray-500 font-medium mb-8">Ingresa tu correo y te enviaremos un enlace para restablecer tu contraseña.</p>
              ...
              {!forgotSent ? (
                <form onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    setForgotSent(true)
                    // MVP: si viene el token, pre-llenarlo en el form de reset
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-2xl px-4 py-4">
                    ...
                    <p className="text-sm font-medium text-green-700">Instrucciones enviadas. Revisa tu correo.</p>
                  </div>
                  {resetToken && (
                    <div className="rounded-2xl border-2 border-gray-200 p-4 space-y-2">
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Token de recuperación (MVP)</p>
                      <p className="text-xs text-gray-500 break-all font-mono">{resetToken}</p>
                      <Button type="button" onClick={() => { setView('reset'); }}
                        className="w-full h-10 bg-[#111111] text-white font-bold text-sm rounded-xl">
              ...
              <button type="button" onClick={() => { setView('login'); setForgotSent(false); setResetToken(''); }}
                className="mt-8 flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-[#111111] transition-colors uppercase tracking-wider">
                <ArrowLeft className="w-4 h-4" /> VOLVER AL LOGIN
              </button>
            </div>
          )}
          ...
          {view === 'reset' && (
            <div className="w-full animate-in fade-in slide-in-from-right-4 duration-300">
              <h2 className="text-3xl font-black text-[#111111] mb-2 tracking-tight">Nueva Contraseña</h2>
              <p className="text-sm text-gray-500 font-medium mb-8">Elige una contraseña segura con letras y números.</p>
              ...
              <form onSubmit={async (e) => {
                ...
                try {
                  ...
                  toast.success("Contraseña actualizada. Entrando...")
                  onLoginSuccess()
                } catch (err: any) {
                ...
                <div className="relative group">
                  ...
                  <button type="button" onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#111111] transition-colors">
                    {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                  </button>
                </div>
                <Button type="submit" disabled={isLoading || !newPassword}
                  className="w-full h-14 bg-[#111111] hover:bg-black text-white font-bold text-lg rounded-2xl shadow-md transition-transform active:scale-[0.98]">
                  {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'GUARDAR CONTRASEÑA'}
                </Button>
              </form>
