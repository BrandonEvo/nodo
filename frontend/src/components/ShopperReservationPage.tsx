...
import { shopperCatalogService, type PublicReservation } from '@/services/shopper_catalog.service';
...
export function ShopperReservationPage({ clientToken }: Props) {
  ...
  useEffect(() => {
    shopperCatalogService.getClientReservation(clientToken)
      .then(setReservation)
      .catch(e => {
    ...
    <div className="min-h-screen bg-nodo-canvas">
      ...
      <div className="max-w-sm mx-auto px-4 py-6 space-y-4">
        ...
        <div className={`rounded-3xl border p-6 flex flex-col items-center text-center gap-3 ${meta.bg}`}>
          <div className="w-16 h-16 rounded-full bg-nodo-card flex items-center justify-center">
            {meta.icon}
          </div>
        ...
        {/* Countdown — solo para reservas activas no vencidas */}
        {isActive && !expired && (
        ...
        <div className="nodo-card p-5 space-y-3">
          ...
          {[
            ...
            reservation.item_price_gtq != null
              ? { label: 'Precio',  value: fmt(reservation.item_price_gtq) }
              : null,
            reservation.deposit_amount != null
              ? { label: 'Depósito acordado', value: fmt(reservation.deposit_amount) }
              : null,
