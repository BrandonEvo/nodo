 ...
 * Si la reserva pertenece a un pedido acumulado, redirige a "En mi maleta".
 */
...
export function ShopperReservationPage({ clientToken }: Props) {
  ...
  useEffect(() => {
    svc.getClientReservation(clientToken)
      .then(r => {
        if (r.order_token) window.location.replace(`/mi-maleta/${r.order_token}`);
    ...
    <div className="min-h-screen bg-nodo-canvas flex flex-col items-center justify-center gap-3 px-6 text-center">
      {error ? (
        ...
        <>
          ...
          <p className="text-sm font-bold text-nodo-sub">Abriendo tu maleta…</p>
        </>
      )}
    </div>
  );
