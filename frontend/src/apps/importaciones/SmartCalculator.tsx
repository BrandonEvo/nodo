...
import {
  Plane, Package2, Settings2, ChevronDown, RotateCcw, TrendingUp,
  Plus, Minus, Info, FileText, Sparkles, Loader2, Calculator,
...
import {
  CATEGORY_DAI_RATE, fmtGTQ, fmtPct,
  type ItemCategory,
} from './pricingEngine';
...
const TAB_OPTS = [
  { value: 'calc',    label: 'Calculadora',  icon: <Calculator size={14} /> },
    ...
    <div className="flex flex-col gap-5 pb-6 w-full max-w-5xl mx-auto">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          >
            <Plane className="w-6 h-6" style={{ color: 'var(--nodo-on-iris)' }} />
          </div>
          <div>
            <h1 className="nodo-module-title">Importaciones</h1>
            <p className="nodo-module-subtitle">Courier GT · cálculo en tiempo real</p>
          </div>
        </div>
        <SegmentedControl
          options={TAB_OPTS}
      ...
      {activeTab === 'quotes' && <CotizacionesTab onNew={() => setActiveTab('calc')} />}
