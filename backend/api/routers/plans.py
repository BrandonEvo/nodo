...
async def create_plan(plan_in: SubscriptionPlanCreate, session: AsyncSession = Depends(get_session)):
...
async def update_plan(plan_id: uuid.UUID, plan_in: SubscriptionPlanUpdate, session: AsyncSession = Depends(get_session)):
